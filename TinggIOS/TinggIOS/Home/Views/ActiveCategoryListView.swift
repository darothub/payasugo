...
struct ActiveCategoryListView: View {
    ...
    var services = Observer<MerchantService>().getEntities()
    var nominations = Observer<Enrollment>().getEntities()
    ...
    fileprivate func onEachCategoryClick(categoryId: String, categoryName: String) {
            ...
            if let defaultService = AppStorageManager.getDefaultNetwork() {
                withAnimation {
                    navigation.navigateTo(
                        screen: BuyAirtimeScreen.buyAirtime(defaultService.serviceName)
                    )
                }
            } else {
                Task {
                    let quickTopups = hvm.getQuickTopups()
                    if let firstService = quickTopups.first {
                        withAnimation {
                            navigation.navigateTo(screen: BuyAirtimeScreen.buyAirtime(firstService.serviceName)
                            )
                        }
                    }
        ...
        default:
            let services = services.filter {$0.categoryID == categoryId}
            let titleAndItemList = TitleAndListItem(title: categoryName, services: services)
            
            _ = nominations.filter {$0.serviceCategoryID == categoryId}
            withAnimation {
                navigation.navigateTo(screen: BillsScreen.billers(titleAndItemList))
            }
        }
    }
}
