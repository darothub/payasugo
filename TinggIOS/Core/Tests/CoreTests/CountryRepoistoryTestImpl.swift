...
import Foundation
import RealmSwift
class CountryRepoistoryTestImpl: CountryRepository {
    ...
    func getRemoteCountries() -> CountryDTO {
        guard let countryDTO = try? JSONDecoder().decode(CountryDTO.self,from: CountryRepoistoryTestImpl.data) else {
